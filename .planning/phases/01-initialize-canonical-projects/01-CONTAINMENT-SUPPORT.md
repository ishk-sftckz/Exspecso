...
## Post-Wave-12 local support correction — measured 2026-08-28

**Superseded history, retained:** earlier planning text described macOS 26.5.1 / build 25F80 as development-only because it was outside the original eight-row ledger. That statement records the then-approved boundary; it is not deleted or rewritten. The user subsequently required this exact machine to become a supported row, and Plan 01-19 added the approved `ENV-MA25` support-row contract.

`ENV-MA25` is now a measured local release-provider result: macOS 26.5.1 build 25F80, arm64, APFS, Node 25.2.1 / live Node-API 10, and a Node-API 8 provider. The maintainer build used Command Line Tools at `/Library/Developer/CommandLineTools`, Apple clang 17.0.0 `clang-1700.6.4.2`, macOS SDK 26.2 build 25C58, and the checksum-verified Node 20.19.0 header archive `800194e32cef4ee77f5a2e8c0e5dd2b4acb38b39ef2ea544929c181b949494bb`.

Plan 01-20's complete unfiltered local suite passed **123/123** tests with zero failures, skips, or todos. The retained source snapshot is `2fe9a678e9ba9b03c7753f921263afae13c95a84`; provider SHA-256 is `dba77dba8c0d15dfe1cf20ee512d86e0b915038b6ccd8666a0296dede07bfc98`; manifest SHA-256 is `d7786781cb1fc6b12ab1280458b51ff2786dfec60c5b85b904af28ab590f48dd`. The record, full-suite report, manifest, and build provenance are retained in `01-20-EVIDENCE/local/`.

The row is exact, not a range: unlisted macOS builds, Node 25.2.0, Node 25.2.2, future Node/macOS releases, unapproved filesystems, or altered CLT/SDK observations are rejected before mutation. A target is not an identity: multiple environment rows can share OS/CPU, so the runtime selects and validates a provider by `supportRowId` plus the row's complete live observation. End users still receive same-package prebuilt providers only; no compiler, headers, lifecycle source build, or provider download is permitted during installation.

The original eight hosted rows remain required. Their Wave 12 records are immutable historical evidence for their own snapshot and cannot be mixed with the new local snapshot. Plan 17 must refresh all nine rows and ten named Node lanes, and Plan 18 must prove one same-final-tarball release candidate before publication/closure claims.
