...
## ENV-MA25 prerequisite and final-candidate gates — 2026-08-28

The prerequisite gate requires the externally retained native `ENV-MA25` record from the current build snapshot, not a GitHub runner label or a hosted substitute. It validates the exact local macOS 26.5.1 / 25F80 arm64/APFS observation, Node 25.2.1 / live Node-API 10, provider Node-API 8, CLT/SDK/header provenance, source, provider, and manifest hashes. Plan 01-20 ran the complete unfiltered command `npm test -- --run --testTimeout 60000` with the release provider present: 123/123 passed, zero failed/skipped/todo. Its retained evidence is `01-20-EVIDENCE/local/`.

The final-candidate gate is stricter and remains pending: it derives its required set from `native/support-matrix.json`, requiring at least the **nine** declared support rows (the eight hosted rows plus ENV-MA25) and all **ten** named Node lanes. It rejects a missing ENV-MA25 or Node 25.2.1 record, any prior missing row/lane, duplicate/conflicting row ID, failed/skipped/cancelled/emulated evidence, wrong row/runtime/live-N-API/provider-N-API/hash/environment, mixed source commits, or a non-identical final-tarball hash. Final aggregation cannot mix the immutable Wave 12 hosted snapshot with the local prerequisite snapshot.

The local prerequisite closes the historical Plan 01-12 full-suite provider gap only. It does not complete package assembly (Plan 16), the all-row/all-lane cross-product and race/recovery matrix (Plan 17), same-final-tarball evidence (Plan 18), independent phase verification, real-TTY UAT, the descriptor-less prohibition acknowledgements, or the security audit.
